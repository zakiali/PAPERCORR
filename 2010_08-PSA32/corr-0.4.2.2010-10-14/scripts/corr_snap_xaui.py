...
try:
    ...
    #bram_dmp=c.snap_x(dev_name,brams,man_trig=man_trigger,wait_period=3)
    bram_dmp=c.snap_x(dev_name,brams,man_trig=man_trigger,wait_period=3,offset=opts.offset*num_bits*2*2/64*packet_len)
    ...
    for f in range((c.config['n_ants']/c.config['n_ants_per_xaui']/c.config['n_xaui_ports_per_fpga'])):
        if len(bram_dmp[brams[2]][f])<=4:
            ...
            if opts.verbose:
                print '\n   Got %i values from %s.'%(len(bram_dmp['bram_oob'][f])/4,c.servers[f])
    ...
    for f in range((c.config['n_ants']/c.config['n_ants_per_xaui']/c.config['n_xaui_ports_per_fpga'])):
        ...
        for i in range(0,len(bram_dmp[brams[2]][f])/4):
            if opts.verbose:
                ...
                print '[%s @ %4i]: %016X'%(c.servers[f],i,pkt_64bit),
    ...
    #            print ('HEADER RECEIVED')
                ...
                print '[%s] [Pkt@ %4i Len: %2i]     (MCNT %16u ANT: %3i, Freq: %4i)  RMS: X: %1.2f Y: %1.2f.  {X: %1.2f+%1.2fj (%2.1f & %2.1f bits), Y:%1.2f+%1.2fj (%2.1f & %2.1f bits)} {Pk: X,Y: %1.2f,%1.2f (%2.1f,%2.1f bits)}'%(c.servers[f],\
                    pkt_hdr_idx,\
