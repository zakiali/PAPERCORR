...
Designed to be connected directly to the output of the 10GbE core. 

Tested working as at 2010-01-17.
...
if __name__ == '__main__':
    ...
    p = OptionParser()
    p.set_usage('snap_dram_rx.py [options] CONFIG_FILE')
...
try:
    ...
    c.write_int_all('snap_dram_rx0_ctrl',0)
    c.write_int_all('snap_dram_rx0_ctrl',1)
    time.sleep(2)
   
    captured_sizes=c.read_uint_all('snap_dram_rx0_addr') 
    ...
    for f,fpga in enumerate(fpgas):
            ...
            if dram_decode[f]['eof'][i]:
                ...
                if prev_eof_index > 0:
                    ...
                    if pkt_len != packet_len:
                        ...
                        first_mcnt = dram_decode[f]['mcnt'][hdr_index]
                        for pkt_index in range(hdr_index,hdr_index+pkt_len):
                            if first_mcnt != dram_decode[f]['mcnt'][pkt_index]:
                                print '[MCNT ERR]',
        ...
        for i in rcvd_mcnts[2:-2]:
            #simulate the reception of the loopback antenna's mcnts:
            for a in range(base_ants[f][0],base_ants[f][0]+c.config['n_ants_per_xaui']): mcnts[f][i][a]=mcnts[f][i].max()
